/*
  ...
  Implementation of k-means algorithm, including k-means++ starting. The
  k-means algorithm clusters data points by seelcting some "means" which become
  the centroids of clusters of points which are closest to that mean. Although
  very fast and surprisingly effective, the choice of "k" can have a dramatic
  effect on the algoithm's output. Commonly, k-means is run multiple times
  with multiple different "k" values. Then the results are compared to find the
  optimal choice of "k" for the data set.
	...
	for (i=1;i<numMeans;i++) {
		...
		_.each(data, function (datum,index) {
			...
			var dist = _.min(_.times(i,
				function (k) {
					return util.distance(datum,means[k]);
			}));
...
var assignClusters = function assignClusters (data, means) {
	...
	_.each(data, function (datum,index) {
		...
		var meanid = _.min(_.zip(means, _.range(means.length)),
			function (mean) { return util.distance(datum,mean[0]); }
		)[1];
