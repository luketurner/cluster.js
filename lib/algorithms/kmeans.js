/*
  ...
  init(inputVars) - initializes module data and gets starting means.
  	Returns an object with "means" member.

  step() - Steps through the algorithm once, clustering the values
  	and updating the means. Returns an object with "means",
  	"clusters", and "round" members, describing the current state of 
  	the algorithm. The object also has a "meansChanged" boolean member.
  	Note: "clusters" contain indecies describing members of original data set,
  	NOT the data members themselves. "means" contains real objects.
  ...
  run(inputVars) - Runs the algorithm to its completion or until the iteration
  	...
  	init(inputVars) and step() for you. Returns an object with "means",
  	"clusters", and "round" members, describing the current state of the algorithm. The
  	object also has a "meansChanged" boolean member (if the limit was reached
  	before the means converged, this will be true. You can continue to run
  	step() manually if you want).
...
var step = function step () {
	...
	round++;
	return {"meansChanged": meansChanged,
		"means": means,
		"clusters": clusters,
		"round": round};
};
